...
Write-Host "`n[3/10] Running RoBERTa sentiment analysis (Day 1)..." -ForegroundColor Yellow
python day1_roberta_sentiment.py
if ($LASTEXITCODE -ne 0) { Write-Error "Day 1 failed."; exit 1 }
Write-Host "      Done." -ForegroundColor Green

# Step 4: Translation
Write-Host "`n[4/10] Running multilingual translation (Day 1.5)..." -ForegroundColor Yellow
python day1.5_translate.py
if ($LASTEXITCODE -ne 0) { Write-Error "Day 1.5 failed."; exit 1 }
Write-Host "      Done." -ForegroundColor Green

# Step 5: Aspect mining + emotion detection
Write-Host "`n[5/10] Running BERTopic aspect mining + emotion detection (Day 2)..." -ForegroundColor Yellow
python day2_aspects.py
if ($LASTEXITCODE -ne 0) { Write-Error "Day 2 failed."; exit 1 }
Write-Host "      Done." -ForegroundColor Green

# Step 6: Business logic
Write-Host "`n[6/10] Running business logic + CEO export (Day 3)..." -ForegroundColor Yellow
python day3_business_logic.py
if ($LASTEXITCODE -ne 0) { Write-Error "Day 3 failed."; exit 1 }
Write-Host "      Done." -ForegroundColor Green

# Step 7: ML suite (XGBoost churn model + anomaly/forecast/semantic search)
Write-Host "`n[7/10] Running ML model suite (Day 4)..." -ForegroundColor Yellow
python day4_ml_models.py
if ($LASTEXITCODE -ne 0) { Write-Error "Day 4 failed."; exit 1 }
Write-Host "      Done." -ForegroundColor Green

# Step 8: Competitor benchmark (skips gracefully if no competitor data)
Write-Host "`n[8/10] Running competitor benchmark (Day 3.5)..." -ForegroundColor Yellow
python day3.5_competitor_benchmark.py
if ($LASTEXITCODE -ne 0) { Write-Error "Competitor benchmark failed."; exit 1 }
Write-Host "      Done." -ForegroundColor Green

# Step 9: AI complaint summary
Write-Host "`n[9/10] Generating AI complaint summary (Day 3.6)..." -ForegroundColor Yellow
python day3.6_ai_complaint_summary.py
if ($LASTEXITCODE -ne 0) { Write-Error "AI complaint summary failed."; exit 1 }
Write-Host "      Done." -ForegroundColor Green

# Step 10: Product identity resolution (brand/title for the Executive Summary tab)
Write-Host "`n[10/10] Resolving product identity (Day 3.7)..." -ForegroundColor Yellow
